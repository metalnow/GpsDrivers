...
int GPSDriverNMEA::handleMessage(int len)
{
	...
	if ((memcmp(_rx_buffer + 3, "ZDA,", 3) == 0) && (uiCalcComma == 6)) {
	...
	} else if ((memcmp(_rx_buffer + 3, "RMC,", 3) == 0)&& (uiCalcComma == 12)) {
		/*
		...
		*/
		char status ='V';
		double nmea_time = 0.0, lat = 0.0, lon = 0.0;
		double ground_speed = 0.0;
		double ground_course = 0.0;
		char ns = '?', ew = '?';
		double utc_time = 0.0, mag_declination = 0.0;
		char mag_dec_dir = '?';

		NMEA_UNUSED(nmea_time);
		NMEA_UNUSED(mag_declination);
		NMEA_UNUSED(mag_dec_dir);
		...
		if (bufptr&& *(++bufptr) != ',') { ground_course = strtod(bufptr, &endp); bufptr =endp; }
		if (bufptr&& *(++bufptr) != ',') { utc_time = strtod(bufptr, &endp); bufptr =endp; }
		if (bufptr&& *(++bufptr) != ',') { mag_declination = strtod(bufptr, &endp); bufptr =endp; }
		if (bufptr&& *(++bufptr) != ',') { mag_dec_dir = *(bufptr++); }
		...
		float track_rad = static_cast<float>(ground_course) * M_PI_F / 180.0f;
		...
		uint8_t fix_type = _gps_position->fix_type;
		if (status == 'V')
			_gps_position->fix_type = 0;
		else
		...
		*/
		...
		if (bufptr &&*(++bufptr) != ',') { ground_course = strtod(bufptr, &endp); bufptr = endp;}
		if (bufptr&& *(++bufptr) != ',') { tmp = strtod(bufptr, &endp); bufptr =endp; }
		if (bufptr&& *(++bufptr) != ',') { tmp = strtod(bufptr, &endp); bufptr =endp; }
		if (bufptr&& *(++bufptr) != ',') { tmp = strtod(bufptr, &endp); bufptr =endp; }
		if (bufptr&& *(++bufptr) != ',') { ground_speed = strtod(bufptr, &endp); bufptr =endp; }
		...
		/*
		  ...
		  $GPGSA,<1>,<2>,<3>,,,,,,,,,,,<15>,<16>,<17>,<18>
		  $GNGSA,A,3,28,23,03,17,19,09,193,06,05,,,,1.4,0.8,1.1*16

		  GSA message fields
		  Field   Meaning
		  0   Message ID $GPGSA/$GNGSA
		  1   模式 ：M = 手動， A = 自動。
		...
		*/
		...
		if (bufptr&& *(++bufptr) != ',') { modeMA = *(bufptr++); }
		if (bufptr&& *(++bufptr) != ',') { mode123 = *(bufptr++); }
		...
		*/
		...
		if ((memcmp(_rx_buffer, "$GP", 3) != 0) && (memcmp(_rx_buffer, "$BD", 3) != 0)) {
			return 0;
...
int GPSDriverNMEA::parseChar(uint8_t b)
{
	...
	case NMEADecodeState::got_sync1:
		...
		{
			_decode_state = NMEADecodeState::got_nmea;
		}
...
int GPSDriverNMEA::configure(unsigned &baudrate, OutputMode output_mode)
{
	...
	{
		GPS_WARN("NEMA:Unsupported Output Mode %i", (int)output_mode);
		return -1;
	}
